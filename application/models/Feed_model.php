...
// $ab=$this->db->query("select feed_comments.comment_id,feed_comments.feed_id,feed_comments.user_id,feed_comments.comment,feed_comments.created_at,feed_comments.updated_at,users.full_name,users.picture from feed_comments Left join users on users.user_id = feed_comments.user_id where feed_comments.feed_id='".$feed_id."' ")->result();
//         // retu$this->db->last_query();
// return $ab;
    }
    ...
    public function FeedByCategory($category_id="",$loginuser_id="",$sort=""){
        ...
        return $data;
         ...
         $data = $this->db->query(" Select feeds.*,users.full_name,users.user_name,users.picture,users.user_type,(select count(1) from feed_likes where feed_likes.feed_id = feeds.feed_id) as totalLikes,(select count(1) from feed_likes where feed_likes.feed_id = feeds.feed_id AND feed_likes.user_id = '".$loginuser_id."') as isLiked,(select count(1) from followers where followers.user_id=feeds.user_id AND follower_id='".$loginuser_id."')as isfollowing,(select count(1) from resharingcount where resharingcount.feed_id = feeds.feed_id) as RepostCount,(select count(1) from resharingcount where resharingcount.feed_id = feeds.feed_id AND  resharingcount.user_id = '".$loginuser_id."') as isRepost,(select count(1) from feed_comments where feed_comments.feed_id = feeds.feed_id) as totalComments FROM feeds Left join users on users.user_id = feeds.user_id where feeds.parent_id =0 AND feeds.status=1 ORDER BY feeds.created_at asc")->result();
               ...
               $data = $this->db->query(" Select feeds.*,users.full_name,users.user_name,users.picture,users.user_type,(select count(1) from feed_likes where feed_likes.feed_id = feeds.feed_id) as totalLikes,(select count(1) from feed_likes where feed_likes.feed_id = feeds.feed_id AND feed_likes.user_id = '".$loginuser_id."') as isLiked,(select count(1) from followers where followers.user_id=feeds.user_id AND follower_id='".$loginuser_id."')as isfollowing,(select count(1) from resharingcount where resharingcount.feed_id = feeds.feed_id) as RepostCount,(select count(1) from resharingcount where resharingcount.feed_id = feeds.feed_id AND  resharingcount.user_id = '".$loginuser_id."') as isRepost,(select count(1) from feed_comments where feed_comments.feed_id = feeds.feed_id) as totalComments FROM feeds Left join users on users.user_id = feeds.user_id where feeds.parent_id =0 AND feeds.status=1 ORDER BY totalLikes desc,feeds.created_at desc")->result();
                return $data; 
            }else{
                 $data = $this->db->query(" Select feeds.*,users.full_name,users.user_name,users.picture,users.user_type,(select count(1) from feed_likes where feed_likes.feed_id = feeds.feed_id) as totalLikes,(select count(1) from feed_likes where feed_likes.feed_id = feeds.feed_id AND feed_likes.user_id = '".$loginuser_id."') as isLiked,(select count(1) from followers where followers.user_id=feeds.user_id AND follower_id='".$loginuser_id."')as isfollowing,(select count(1) from resharingcount where resharingcount.feed_id = feeds.feed_id) as RepostCount,(select count(1) from resharingcount where resharingcount.feed_id = feeds.feed_id AND  resharingcount.user_id = '".$loginuser_id."') as isRepost,(select count(1) from feed_comments where feed_comments.feed_id = feeds.feed_id) as totalComments FROM feeds Left join users on users.user_id = feeds.user_id where feeds.parent_id =0 AND feeds.status=1 ORDER BY feeds.created_at desc")->result();
                return $data;        
...
//this function is used to get the feed by popularity by user_id
    public function getMyFeedByPopularity($userid="",$user_type="",$loginuser_id=""){
        ...
        $data=$this->db->query(" Select feeds.*,users.full_name,users.user_name,users.picture,users.user_type,(select count(1) from feed_likes where feed_likes.feed_id = feeds.feed_id) as totalLikes,(select count(1) from feeds where feeds.author_id = '".$userid."' AND feeds.parent_id=0) as totalQuotes,(select count(1) from resharingcount where resharingcount.feed_id = feeds.feed_id) as RepostCount,(select count(1) from resharingcount where resharingcount.feed_id = feeds.feed_id AND  resharingcount.user_id ='".$loginuser_id."') as isRepost,(select count(1) from feed_likes where feed_likes.feed_id = feeds.feed_id AND feed_likes.user_id = '".$loginuser_id."') as isLiked,(select count(1) from followers where followers.user_id=feeds.user_id AND follower_id='".$loginuser_id."')as isfollowing,(select count(1) from feed_comments where feed_comments.feed_id = feeds.feed_id) as totalComments,(select full_name from users where users.user_id=feeds.author_id)as author_name,(select full_name from users where user_id=feeds.booker_id)as book_name FROM feeds Left join users on users.user_id = feeds.user_id where feeds.author_id='".$userid."' And feeds.parent_id=0 AND feeds.status=1 ORDER BY totalLikes desc")->result();
        return $data;
    }elseif ($user_type==3) {
        $data=$this->db->query(" Select feeds.*,users.full_name,users.user_name,users.picture,users.user_type,(select count(1) from feed_likes where feed_likes.feed_id = feeds.feed_id) as totalLikes,(select count(1) from feeds where feeds.booker_id = '".$userid."' AND feeds.parent_id=0) as totalQuotes,(select count(1) from resharingcount where resharingcount.feed_id = feeds.feed_id) as RepostCount,(select count(1) from resharingcount where resharingcount.feed_id = feeds.feed_id AND  resharingcount.user_id ='".$loginuser_id."') as isRepost,(select count(1) from feed_likes where feed_likes.feed_id = feeds.feed_id AND feed_likes.user_id = '".$loginuser_id."') as isLiked,(select count(1) from followers where followers.user_id=feeds.user_id AND follower_id='".$loginuser_id."')as isfollowing,(select count(1) from feed_comments where feed_comments.feed_id = feeds.feed_id) as totalComments,(select full_name from users where users.user_id=feeds.author_id)as author_name,(select full_name from users where user_id=feeds.booker_id)as book_name FROM feeds Left join users on users.user_id = feeds.user_id where feeds.booker_id='".$userid."' And feeds.parent_id=0 AND feeds.status=1 ORDER BY totalLikes desc")->result();
           return $data;

    }else{
        $data=$this->db->query(" Select feeds.*,users.full_name,users.user_name,users.picture,users.user_type,(select count(1) from feed_likes where feed_likes.feed_id = feeds.feed_id) as totalLikes,(select count(1) from feeds where(find_in_set('".$userid."',feeds.tagger_id)) AND feeds.parent_id=0) as totalQuotes,(select count(1) from resharingcount where resharingcount.feed_id = feeds.feed_id) as RepostCount,(select count(1) from resharingcount where resharingcount.feed_id = feeds.feed_id AND  resharingcount.user_id ='".$loginuser_id."') as isRepost,(select count(1) from feed_likes where feed_likes.feed_id = feeds.feed_id AND feed_likes.user_id = '".$loginuser_id."') as isLiked,(select count(1) from followers where followers.user_id=feeds.user_id AND follower_id='".$loginuser_id."')as isfollowing,(select count(1) from feed_comments where feed_comments.feed_id = feeds.feed_id) as totalComments,(select full_name from users where users.user_id=feeds.author_id)as author_name,(select full_name from users where user_id=feeds.booker_id)as book_name FROM feeds Left join users on users.user_id = feeds.user_id where (find_in_set('".$userid."',feeds.tagger_id)) And feeds.parent_id=0 AND feeds.status=1 ORDER BY totalLikes desc")->result();
           return $data;
    }
    ...
    public function updateRepost($feed_id,$data){
        ...
        #________
    // This function is used to get the all data
  public function ShowBannerById($banner_id=""){
    $q=$this->db->query("select * from banner WHERE banner_categoryid='".$banner_id."'  and status=1 ORDER by is_feature DESC")->result();
        if($q)
        {
           return array('status' => 200,'message'=>'Banner.','data'=>$q);
        }else{
              //return false;
            return array('status' => 200,'message'=>'No banner found.','data'=>array());
        }
  }
  // 

   public function getAnyTable($table,$field,$cond){
        $data = $this->db->select($field)->from($table)->where($cond)->get()->result_array();
        //$this->db->last_query();
        return($data);
    }

    public function getCountAnything($table,$field,$cond){
